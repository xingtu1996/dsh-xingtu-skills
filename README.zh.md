***REMOVED*** dsh-xingtu-skills · XingTu 技能插件（DeepSeek Harness / DSH）
...
***REMOVED******REMOVED*** 插件结构

```
dsh-xingtu-skills/
├── package.json        ***REMOVED*** dsh.bundle.patch → ./cordis.patch.yml（标准插件声明）
├── cordis.patch.yml    ***REMOVED*** 插入本插件行（bundle layer）
├── lib/index.js        ***REMOVED*** 入口：apply() 逐个注册 skills/ 下的 SKILL.md
└── skills/<name>/      ***REMOVED*** 26 个技能目录（含各自 SKILL.md 与资源）
```
